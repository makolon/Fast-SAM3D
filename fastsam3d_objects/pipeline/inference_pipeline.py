...
import time
from fastsam3d_objects.fft.fft2d import calculate_slope_robust, calculate_hfer_robust, preprocess_for_fft_masked
from fastsam3d_objects.fft.fft3d import get_coords_value, analyze_voxel_frequency, plot_spatial_heatmap, plot_freq_domain, process_and_visualize, plot_coords_scores_to_html
...
from fastsam3d_objects.pipeline import preprocess_utils

from fastsam3d_objects.data.dataset.tdfy.img_and_mask_transforms import (
    get_mask,
)
from fastsam3d_objects.pipeline.inference_utils import (
    get_pose_decoder,
    SLAT_MEAN,
    SLAT_STD,
    downsample_with_feature_fusion,
    calculate_adaptive_factor,

    prune_sparse_structure,
)

from fastsam3d_objects.model.io import (
    load_model_from_checkpoint,
    filter_and_remove_prefix_state_dict_fn,
)

from fastsam3d_objects.model.backbone.tdfy_dit.modules import sparse as sp
from fastsam3d_objects.model.backbone.tdfy_dit.utils import postprocessing_utils
...
class InferencePipeline:
    ...
    def postprocess_slat_output(
        ...
        logger.info(
            f"Postprocessing mesh with option with_meshpostprocess {with_mesh_postprocess}, with_texture_baking {with_texture_baking},use_vertex_color {use_vertex_color}")
    ...
    def decode_slat(
        ...
        with torch.no_grad():
            if "mesh" in formats:
                m_s_time = time.time()
                self.models["slat_decoder_mesh"].map = map 

                ret["mesh"] = self.models["slat_decoder_mesh"](slat)

                m_e_time = time.time()
                diff_m_time = m_e_time- m_s_time

            if "gaussian" in formats:
                g_s_time = time.time()
                ret["gaussian"] = self.models["slat_decoder_gs"](slat)
                g_e_time = time.time()
                diff_g_time = g_e_time- g_s_time

            if "gaussian_4" in formats:
                ret["gaussian_4"] = self.models["slat_decoder_gs_4"](slat)


        logger.info(f"decoder finish !  mesh: {diff_m_time:.2f} , gs : {diff_g_time:.2f}")
        return ret
    ...
    def sample_sparse_structure(
        self, ss_input_dict: dict, inference_steps=None, use_distillation=False,hfer_2d =0, ss_params = None,
    ):
        logger.info("sample_sparse_structurer begin !")
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=self.shape_model_dtype):
                ...
                print(self.ss_condition_input_mapping)
                ...
                logger.info("ss generator begin !")
                ss_time= time.time()
                ss_generator.decoder = ss_decoder

                ss_generator.ss_params = self.ss_params
                ...
                end_time= time.time()
                diff_time = end_time-ss_time
                logger.info(f"ss generator finish !: {diff_time:.2f}")
                ...
                logger.info("ss decoder begin !")
                ss = ss_decoder(
                    shape_latent.permute(0, 2, 1)
                    .contiguous()
                    .view(shape_latent.shape[0], 8, 16, 16, 16)
                )
                logger.info("ss decoder finish !")
        ...
        k_map_shap = getattr(ss_generator, 'k_map_shap', None)
        return return_dict,k_map_shap,coords_scores
    ...
    def sample_slat(
        ...
        with torch.autocast(device_type="cuda", dtype=self.dtype):
            with torch.no_grad():
                ...
                logger.info("slat generator begin !")

                slat_generator.slat_params = self.slat_params
