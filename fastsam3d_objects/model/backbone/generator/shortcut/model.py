...
from fastsam3d_objects.model.backbone.generator.base import Base
from fastsam3d_objects.data.utils import right_broadcasting
from fastsam3d_objects.data.utils import tree_tensor_map, tree_reduce_unique
from fastsam3d_objects.model.backbone.generator.flow_matching.model import FlowMatching, _get_device
...
from fastsam3d_objects.step_utils_ss import derivative_approximation, step_formula, step_cache_init
from fastsam3d_objects.cache_utils_ss import cal_type, cache_init
class ShortCut_faster(ShortCut):
    ...
    def forward(self, x_shape, x_device, *args_conditionals, **kwargs_conditionals):
        ...
        self.cache_dic, self.current = cache_init( self.inference_steps ,
                                                    cache_interval = self.ss_params['ss_cache_stride'],
    ...
    def _generate_dynamics(
        ...
        **kwargs_conditionals,
        ...
        step_cache_init(self.cache_dic, self.current)

        if self.current['type'] == 'full':
            v = self.reverse_fn(x_t, t, *args_conditionals, d=d, **kwargs_conditionals)
            self.prev_v = v
            derivative_approximation(self.cache_dic, self.current, v)

        elif self.current['type'] == 'cache':
            v = step_formula(self.cache_dic, self.current, self.prev_v, beta = self.ss_params['ss_momentum_beta'])
            self.prev_v = v
